...
- If the shutdown Lambda errors when invoked, the `bluths-api-budget-shutdown-errors` CloudWatch Alarm publishes to the `bluths-api-budget-alerts` SNS topic so the operator is notified of silent failures.
- On the 1st of each month at 00:05 UTC, the `bluths-api-month-rollover-check` Lambda runs. If the API is still disabled (throttle still 0), it emails a reminder via the alerts topic — so the operator doesn't enter the new month unaware. The rollover Lambda is read-only; it does not modify state.
...
To keep budget alerts but remove automatic shutdown, unsubscribe the Lambda from the shutdown-trigger SNS topic. The $20/$30 notifications continue to email the operator; only the kill switch is muted.

```bash
SUB_ARN=$(aws sns list-subscriptions-by-topic \
  --topic-arn arn:aws:sns:us-east-1:<ACCOUNT_ID>:bluths-api-budget-shutdown-trigger \
...
- Review SNS topic subscriptions: `aws sns list-subscriptions-by-topic --topic-arn arn:aws:sns:us-east-1:<ACCOUNT_ID>:bluths-api-budget-shutdown-trigger`
